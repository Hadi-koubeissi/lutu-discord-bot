...
class Setting extends Command {
  ...
  async run (message, args, level, reply) { // eslint-disable-line no-unused-vars
    ...
    const settingEmbed = new Discord.MessageEmbed()
      ...
      .addField("[General]:", `▫ Prefix: \`|${settings.prefix}|\` Do not include the 2x |.\n▫ Moderation Log Channel: ${message.guild.channels.cache.get(settings.logsChannel) || "NONE"}\n▫ Under Attack: ${settings.underAttack.toUpperCase()}`)
      .addField("[Roles]:", `▫ Moderator Role: ${message.guild.roles.cache.get(settings.modRole) || "NONE"}\n▫ Administrator Role: ${message.guild.roles.cache.get(settings.adminRole) || "NONE"}\n▫ Auto Join Role: ${message.guild.roles.cache.get(settings.joinrole) || "NONE"}`)
      ...
      .addField("[Auto-Moderator Ignored]:", `▫ Ignored Channels: ${settings.ignoredChannels.map(c=>`${message.guild.channels.cache.get(c)}`).join(", ")}\n▫ Roles: ${settings.ignoredRoles.map(r=>`${message.guild.roles.cache.get(r)}`).join(", ")}\n▫ Members: ${settings.ignoredUsers.map(u=>`${this.client.users.cache.get(u)}`).join(", ")}`)
      .addField("[Checkpoint]:", `▫ Status: ${settings.checkpoint.toUpperCase()}\n▫ Mode: ${settings.checkpoint_mode.toUpperCase()}\n▫ Channel: ${message.guild.channels.cache.get(settings.checkpoint_logChannel) || "NONE"}`)
